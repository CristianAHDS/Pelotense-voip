...
- [x] **16. Indicador de digitação** — Mensagem `typing` com debounce. ✅ Feito: `Typing` no protocolo (servidor repassa para a sala exceto o autor, com throttle de 2s por usuário); o cliente envia o sinal enquanto digita (debounce de 3s) e mostra "X está digitando…" acima do input com auto-expiração de 4s (timers no `connectionService` + store `typing` no `roomStore`); disponível em PT/EN.

- [x] **19. Limpar código morto** — `udpServer.ts`, `voice/router.ts`, `packets/*`, `zod`, `EventBus`, `udpClient.ts`, `settingsStore` não usados. Remover reduz superfície de bugs. ✅ Feito: removidos `network/udpServer.ts`, `voice/router.ts`, `voice/index.ts`, `packets/*`, `utils/events.ts` (EventBus + emissões órfãs em `RoomManager`/`WsHandler`), `network/udpClient.ts`, dependência `zod`; eliminado o `udpPort` morto (config, `Client`, welcome payload e tipos do cliente); `settingsStore` enxugado (removidos `serverHost`/`serverWsPort` não usados, mantidos tema/idioma).
...
| 03/08/2026 | Item 16 (indicador de digitação) e item 19 (limpeza de código morto). ✅ Feito: `Typing` no protocolo com relay/throttle no servidor e UI "X está digitando…" no client (debounce); removidos `udpServer.ts`, `voice/router.ts`, `voice/index.ts`, `packets/*`, `utils/events.ts` (EventBus), `udpClient.ts`, dependência `zod` e o `udpPort` morto; `settingsStore` enxugado. TODOs pendentes (12, 17, 18, R1, R4, R5, R6, R7, R9, V2.5–V2.8, V2.10, V2.11, V2.13) removidos do arquivo. Testes: server 141 ✓, client 279 ✓. | ✅ Feito |
