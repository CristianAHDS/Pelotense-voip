...
- [x] **12. Push-to-talk** — O store (`pushToTalk`/`pushToTalkKey`) já existe mas não está conectado. Fácil de ativar (tecla + indicador visual).(deve ser feito dessa forma no mobile: uma opção na tela ara ativar / desativar o push tot alk, ao ficar ativo, deve aparecer no meio da tela na parte de baixo um botão onde precionado ele libera o microfone, ao largar, ele para de enviar audio) ✅ Feito: estado `transmitting` no voiceStore, gate de envio no VoiceManager, botão flutuante de segurar-para-falar (`PushToTalkButton`) no mobile, toggle + seletor de tecla (Espaço/V/C/T/Ctrl) no painel e botão PTT na barra compacta; testes cobrindo gate de envio, botão e alternância.
